...
type UsersFormGroupContent = {
  ...
  userName: FormControl<IUsers['userName']>;
  wallets: FormControl<IUsers['wallets']>;
  galleries: FormControl<IUsers['galleries']>;
  password: FormControl<IUsers['password']>;
  type: FormControl<IUsers['type']>;
};
  ...
  createUsersFormGroup(users: UsersFormGroupInput = { id: null }): UsersFormGroup {
    ...
    return new FormGroup<UsersFormGroupContent>({
      ...
      userName: new FormControl(usersRawValue.userName, {
        validators: [Validators.required, Validators.minLength(1), Validators.maxLength(64), Validators.pattern('^[A-Z][a-z]+\\d$')],
      }),
      wallets: new FormControl(usersRawValue.wallets),
      galleries: new FormControl(usersRawValue.galleries),
      password: new FormControl(usersRawValue.password, {
        validators: [Validators.required],
      }),
      type: new FormControl(usersRawValue.type),
    });
  }
